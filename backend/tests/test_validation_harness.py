...
class TestBenchmarkDiscovery:
    ...
    def test_exactly_20_sample_files(self):
        samples = [f for f in os.listdir(BENCHMARK_DIR)
                    if f.startswith("sample") and f.endswith(".py")]
        assert len(samples) == 20, f"Expected 20 sample files, found {len(samples)}: {sorted(samples)}"

    def test_sample_files_named_correctly(self):
        expected = {f"sample{i:02d}.py" for i in range(1, 21)}
        actual = {f for f in os.listdir(BENCHMARK_DIR)
                  if f.startswith("sample") and f.endswith(".py")}
    ...
    def test_model_properties_files_exist(self):
        expected_files = [
            "confidence_base.py",
            "severity_low.py", "severity_medium.py", "severity_high.py", "severity_critical.py",
            "size_500.py", "size_1000.py", "size_2000.py",
            "complexity_low.py", "complexity_medium.py", "complexity_high.py",
        ]
...
class TestGroundTruth:
    ...
    def test_ground_truth_file_exists(self):
        assert os.path.isfile(GROUND_TRUTH_CSV), f"Ground truth CSV missing: {GROUND_TRUTH_CSV}"
    ...
    def test_ground_truth_required_columns(self):
        rows = self._load_ground_truth()
        required = {
            "file", "loc_expected", "expected_security_level",
            "expected_performance_level", "expected_maintainability_level",
            "known_issue_summary", "ground_truth_source",
        }
    ...
    def test_every_ground_truth_maps_to_benchmark_file(self):
        ...
        benchmark_files = {f for f in os.listdir(BENCHMARK_DIR)
                          if f.startswith("sample") and f.endswith(".py")}
        for row in rows:
            assert row["file"] in benchmark_files, \
    ...
    def test_valid_severity_labels(self):
        ...
        for row in rows:
            for col in ["expected_security_level", "expected_performance_level",
                        "expected_maintainability_level"]:
                assert row[col] in valid_labels, \
...
class TestValidationPipeline:
    ...
    def test_all_benchmark_files_produce_valid_scores(self):
        ...
        score_fields = [
            ...
            "technical_debt_score", "overall_quality",
        ]
        ...
        for fname in sorted(os.listdir(BENCHMARK_DIR)):
            if fname.startswith("sample") and fname.endswith(".py"):
                ...
                assert scores["maintainability_grade"] in ("A", "B", "C", "D", "F"), \
    ...
    def test_results_contain_scoring_metadata(self):
        ...
        for key in ["total_impact", "normalized_impact", "confidence_adjusted_issue_count",
                     "normalized_issue_density", "average_complexity", "loc_k",
                     "lines_of_code", "num_functions", "raw_issue_count"]:
    ...
    def test_clean_code_scores_high(self):
        ...
        for i in range(1, 5):
            ...
            if os.path.exists(filepath):
                ...
                assert scores["overall_quality"] >= 50, \
