...
def test_scoring_ai_independence():
    ...
    score_1 = scoring.calculate_scores(
        issues=unified_scenario_1, cyclomatic_complexity=2, lines_of_code=10
    )
    ...
    score_2 = scoring.calculate_scores(
        issues=unified_scenario_2, cyclomatic_complexity=2, lines_of_code=10
    )
    ...
    score_3 = scoring.calculate_scores(
        issues=unified_scenario_3, cyclomatic_complexity=2, lines_of_code=10
    )
    ...
    assert (
        score_1 == score_2
    ), "Score must be identical if static issues are identical and no AI issues are found"

    # 2. Score changes when genuine AI issues are added
    assert (
        score_1["overall_quality"] > score_3["overall_quality"]
    ), "Overall quality should drop with more issues"
    assert (
        score_1["maintainability_score"] > score_3["maintainability_score"]
    ), "Maintainability should drop with more issues"
    assert (
        score_1["technical_debt_score"] > score_3["technical_debt_score"]
    ), "Tech debt score should drop with more issues"

    # Print exact scoring data passed into the Scoring Engine for the report
    print("\n--- SCORING AUDIT REPORT ---")
    print(
        f"Scenario 1 (Gemini unavailable) Data into Scoring Engine: {len(unified_scenario_1)} issues, Complexity 2"
    )
