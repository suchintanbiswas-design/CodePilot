...
async def test_download(
    client: AsyncClient, override_get_db, auth_override, mock_review_id
):
    with (
        patch(
            "app.repositories.review_repository.ReviewRepository.get",
            new_callable=AsyncMock,
        ) as mock_get,
        patch(
            "app.services.report_service.ReportService.get_or_generate_report",
            new_callable=AsyncMock,
        ) as mock_gen,
    ):
...
async def test_favorite(
    ...
    with patch(
        "app.services.favorite_service.FavoriteService.add_review_to_collection",
        new_callable=AsyncMock,
    ) as mock_fav:
...
async def test_duplicate(
    ...
    with patch(
        "app.services.review_service.ReviewService.duplicate_review",
        new_callable=AsyncMock,
    ) as mock_dup:
        from app.models.review import Review
...
async def test_delete(
    ...
    with patch(
        "app.services.review_service.ReviewService.delete_review",
        new_callable=AsyncMock,
    ) as mock_del:
