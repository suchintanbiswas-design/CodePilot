...
async def test_gemini_provider_usage_metadata():
    ...
    mock_response.text = (
        '{"ai_summary": "Test", "improved_code": "Test", "ai_enhanced_issues": []}'
    )
...
def test_dashboard_tokens_sum_and_exclusion():
    ...
    r1 = Review(
        user_id=user1_id,
        status="completed",
        review_metadata={"ai_usage": {"total_tokens": 1000}},
    )
    r2 = Review(
        user_id=user1_id,
        status="completed",
        review_metadata={"ai_usage": {"total_tokens": 2500}},
    )
    Review(
        user_id="2",
        status="completed",
        review_metadata={"ai_usage": {"total_tokens": 5000}},
    )
    r4 = Review(
        user_id=user1_id,
        status="failed",
        review_metadata={"ai_usage": {"total_tokens": 500}},
    )
    ...
    assert (
        ai_usage_tokens == 3500
    )  # Excludes failed r4, excludes user2's r3 by definition
...
def test_tech_debt_trend():
    ...
    reviews2 = [
        Review(
            ...
            review_metadata={"tech_debt": 100},
        ),  # earliest avg 100
        Review(
            status="completed", created_at=now, review_metadata={"tech_debt": 70}
        ),  # latest avg 70
    ]
    ...
    reviews3 = [
        ...
        Review(
            status="completed", created_at=now, review_metadata={"tech_debt": 125}
        ),  # latest avg 125
    ]
