...
def test_gemini_model_configuration():
    ...
    ), "The configured model must be gemini-3.5-flash-lite"
...
async def test_gemini_provider_response_parsing():
    ...
    class MockResponse:
        text: str = (
            '{"ai_summary": "Great code", "improved_code": "def test(): return True", "ai_enhanced_issues": []}'
        )
        usage_metadata: MockUsageMetadata = dataclasses.field(
            default_factory=MockUsageMetadata
        )

    provider.client = MagicMock()
    provider.client.aio.models.generate_content = AsyncMock(return_value=MockResponse())

    summary, improved, issues, usage = await provider.analyze_code(
        "def test(): pass", []
    )
...
async def test_gemini_success():
    ...
    review = Review(
        ...
        review_metadata={"requested_language": "Python"},
    )

    with (
        patch(
            "app.engine.ai_reviewer.AIReviewer.review", new_callable=AsyncMock
        ) as mock_review,
        ...
        patch(
            "app.engine.language_detector.LanguageDetector.validate_language"
        ) as mock_lang,
    ):
...
async def test_gemini_rate_limit():
    ...
    with (
        ...
        mock_static.return_value = [
            {
                "line_number": 1,
                "description": "Static ok",
                "severity": "Low",
                "rule_type": "Style",
            }
        ]

        mock_review.side_effect = AIAvailabilityError(
            "rate_limit", "Gemini API error: 429 Too Many Requests"
        )
...
async def test_gemini_provider_error():
    ...
    with (
        ...
        mock_review.side_effect = AIAvailabilityError(
            "provider_error", "Gemini API error: 500 Internal Server Error"
        )
