...
async def test_language_persistence_valid_language():
    ...
    with patch("app.services.review_service.select"):
...
async def test_language_persistence_switch_to_python():
    ...
    req = ReviewCreateRequest(
        title="Test Python", language_id=str(python_id), source_code="x=1"
    )
...
async def test_language_persistence_unknown_language():
    ...
    service = ReviewService(review_repo=None)
    req = ReviewCreateRequest(
        title="Test Unknown", language_id=req_lang_id, source_code="x=1"
    )
...
async def test_auto_detect_language_success():
    ...
    with patch(
        ...
        assert review.language_id == python_id
        assert (
            review.review_metadata["language_detection"]["final_language"] == "Python"
        )
        assert (
            review.review_metadata["language_detection"]["language_switched"] is False
        )
...
async def test_auto_detect_language_low_confidence_fails():
    ...
    with patch(
        ...
        with pytest.raises(
            Exception, match="Could not confidently determine programming language"
        ):
            await service._process_single_file(db_mock, review)
