...
async def create_and_wait(client, code, ext, title):
    ...
    resp = await client.post(
        f"{BASE_URL}/reviews", data={"req_data": json.dumps(req_data)}
    )
    ...
    print(
        f"Token Tracking: {'ai_usage' in data.get('metadata', {}) or 'usage_metadata' in data.get('metadata', {})}"
    )
...
async def main():
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        login = await client.post(
            ...
            json={"email": "admin@codepilot.dev", "password": "Admin@123456"},
        )
