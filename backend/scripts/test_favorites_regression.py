...
def main():
    ...
    r = requests.post(
        ...
        data={
            "req_data": json.dumps(
                {"title": "Test Regr", "language_id": "Python", "source_code": "x=1"}
            )
        },
        headers=headers,
    )
    ...
    if my_favs:
        ...
        col_id = my_favs["id"]
        r = requests.get(
            f"{BASE}/favorites/collections/{col_id}/reviews", headers=headers
        )
    ...
    # 4. Unfavorite
    requests.delete(
        f"{BASE}/favorites/collections/{col_id}/reviews/{review_id}", headers=headers
    )
