...
def test_api():
    ...
    login_data = {"email": "admin@codepilot.dev", "password": "Admin@123456"}
    ...
    if r_py.status_code == 202:
        ...
        if data.get("language") and data["language"].get("name") == "Python":
            print(
                "  => VERIFIED: Python language is eager loaded and serialized correctly!"
            )
        else:
    ...
    if r_java.status_code == 202:
        ...
        if data.get("language") and data["language"].get("name") == "Java":
            print(
                "  => VERIFIED: Java language is eager loaded and serialized correctly!"
            )
        else:
            print("  => ERROR: language object is missing or incorrect!")
    else:
