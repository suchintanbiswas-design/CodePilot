...
async def main():
    ...
    async with async_session_maker() as db:
        ...
        for test_lang in test_inputs:
            ...
            if found:
                print(
                    f"  Lookup '{test_lang}' => FOUND: id={found.id}, name={found.name}"
                )
            else:
                print(
                    f"  Lookup '{test_lang}' => NOT FOUND (will become language_id=None)"
                )
        ...
        for r in reviews:
            ...
            print(
                f"    metadata.requested_language={r.review_metadata.get('requested_language', 'NOT SET')}"
            )
            lang_det = r.review_metadata.get("language_detection", {})
            print(
                f"    metadata.language_detection.selected={lang_det.get('selected_language', 'N/A')}"
            )
            print(
                f"    metadata.language_detection.detected={lang_det.get('detected_language', 'N/A')}"
            )
            print(
                f"    metadata.language_detection.is_match={lang_det.get('is_match', 'N/A')}"
            )
            print()
        ...
        if reviews:
            ...
            if latest.language_id:
                ...
                print(
                    f"  metadata.requested_language = {latest.review_metadata.get('requested_language', 'NOT SET')}"
                )
