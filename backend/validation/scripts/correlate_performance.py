...
def main():
    ...
    with open(out_csv, "w", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "expected_performance_rank",
                "codepilot_performance_score",
                "codepilot_performance_rank",
                "rank_difference",
            ],
        )
        writer.writeheader()
    ...
    for x in sorted(joined, key=lambda x: x["expected_performance_rank"], reverse=True)[
        :5
    ]:
        print(
            f"{x['file']}: {x['expected_performance_level_str']} (Rank {x['expected_performance_rank']})"
        )
    ...
    for x in sorted(joined, key=lambda x: x["true_disagreement"], reverse=True)[:5]:
        print(
            f"{x['file']}: Diff={x['true_disagreement']} | GT={x['expected_performance_level_str']} (Rank {x['expected_performance_rank']}) vs "
            f"CP Score={x['codepilot_performance_score']} (Aligned Rank {x['codepilot_performance_rank']})"
        )
        print(
            f"   LOC: {x['loc']}, Issues: {x['issue_count']}, Complexity: {x['complexity']}"
        )
