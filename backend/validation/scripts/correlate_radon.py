...
def main():
    ...
    with open(codepilot_csv, "r") as f:
        ...
        for row in reader:
            if "sample" in row["file"] and "model_properties" not in row["file"] and row["file"].startswith("sample"):
    ...
    for f in sorted(cp_files):
        joined.append({
            "file": f,
            "loc": int(cp_data[f]["loc"]) if "loc" in cp_data[f] else int(radon_data[f]["loc"]),
            "codepilot_maintainability": float(cp_data[f]["maintainability_score"]),
            "radon_maintainability_index": float(radon_data[f]["maintainability_index"]),
    ...
    with open(out_csv, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "file", "loc", "codepilot_maintainability", "radon_maintainability_index",
            "codepilot_rank", "radon_rank", "rank_difference"
        ])
        writer.writeheader()
        for x in joined:
            writer.writerow({
    ...
    print(f"Largest rank disagreement: {max_diff_item['file']} (diff={max_diff_item['abs_rank_difference']})")
    
    # Top 5 and Bottom 5
    print("\n--- CodePilot Top 5 ---")
    for x in sorted(joined, key=lambda x: x["codepilot_rank"])[:5]:
        print(f"  Rank {x['codepilot_rank']}: {x['file']} (Score {x['codepilot_maintainability']})")
    ...
    for x in sorted(joined, key=lambda x: x["radon_rank"])[:5]:
        print(f"  Rank {x['radon_rank']}: {x['file']} (MI {x['radon_maintainability_index']:.2f})")
    ...
    for x in sorted(joined, key=lambda x: x["abs_rank_difference"], reverse=True)[:5]:
        print(f"{x['file']}: diff={x['abs_rank_difference']} "
