...
def main():
    ...
    for i in range(1, 21):
        ...
        raw_key = next((k for k in raw_data.keys() if k.replace('\\', '/').endswith(filename)), None)
        mi_key = next((k for k in mi_data.keys() if k.replace('\\', '/').endswith(filename)), None)
        cc_key = next((k for k in cc_data.keys() if k.replace('\\', '/').endswith(filename)), None)

        if not (raw_key and mi_key and cc_key):
            print(f"Missing data for {filename}. Raw: {bool(raw_key)}, MI: {bool(mi_key)}, CC: {bool(cc_key)}")
            continue
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "file", "loc", "maintainability_index", "maintainability_grade",
            "total_cyclomatic_complexity", "function_count", "average_cyclomatic_complexity"
        ])
        writer.writeheader()
