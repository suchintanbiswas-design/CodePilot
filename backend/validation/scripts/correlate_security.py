...
def main():
    ...
    for i in range(N):
        ...
        joined[i]["codepilot_security_rank_aligned"] = cp_ranks[i] # already lower value = lower score = lower rank number, wait.
    ...
    with open(out_csv, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "file", "expected_security_level", "expected_security_rank",
            "codepilot_security_score", "codepilot_security_rank", "rank_difference"
        ])
        writer.writeheader()
        for x in joined:
            writer.writerow({
    ...
    for x in sorted(joined, key=lambda x: x["expected_security_rank"], reverse=True)[:5]:
        print(f"{x['file']}: {x['expected_security_level_str']} (Rank {x['expected_security_rank']})")
    ...
    for x in sorted(joined, key=lambda x: x["true_disagreement"], reverse=True)[:5]:
        print(f"{x['file']}: Diff={x['true_disagreement']} | GT={x['expected_security_level_str']} (Rank {x['expected_security_rank']}) vs "
