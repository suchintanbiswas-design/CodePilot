...
class JavaSemanticAnalyzer:
    ...
    def analyze(self, code: str) -> List[Dict[str, Any]]:
        ...
        issues.extend(
            self._check_string_reference_comparison(lines, string_vars, string_getters)
        )

        # ── Rule 3: Floating-point currency ──────────────────────────
        issues.extend(self._check_floating_point_currency(lines, is_financial_context))

        return issues

    # ── Discovery helpers ─────────────────────────────────────────────

    def _discover_classes(self, lines: list[str]) -> dict[str, dict]:
        ...
        for line in lines:
            ...
            m_class = re.search(r"\bclass\s+(\w+)", clean)
            if m_class:
            ...
            if current_class and current_class in classes:
                ...
                if re.search(r"\bboolean\s+equals\s*\(\s*Object\s+", clean):
                    info["has_equals_object"] = True

                # public boolean equals(SpecificType ...)
                m_eq = re.search(r"\bboolean\s+equals\s*\(\s*(\w+)\s+", clean)
                if m_eq:
    ...
    def _discover_string_variables(self, lines: list[str]) -> set[str]:
        ...
        for line in lines:
            ...
            for m in re.finditer(r"\bString\s+(\w+)\s*[;=,)]", clean):
                string_vars.add(m.group(1))
            # Method parameters: (String paramName, ...)
            for m in re.finditer(r"String\s+(\w+)", clean):
                string_vars.add(m.group(1))
        return string_vars
    ...
    def _discover_string_getters(self, lines: list[str]) -> set[str]:
        ...
        for line in lines:
            ...
            m = re.search(r"\bString\s+(\w+)\s*\(", clean)
            if m:
                name = m.group(1)
    ...
    def _check_string_reference_comparison(
        ...
        for i, line in enumerate(lines):
            ...
            for m in re.finditer(r"(\b[\w.()]+)\s*(==|!=)\s*([\w.()]+\b)", clean):
                lhs = m.group(1).strip()
                op = m.group(2)
                rhs = m.group(3).strip()

                if self._is_string_expr(
                    lhs, string_vars, string_getters
                ) and self._is_string_expr(rhs, string_vars, string_getters):
                    issues.append(
                        {
                            "severity": "High",
                            "category": "Correctness",
    ...
    def _check_floating_point_currency(
        ...
        for i, line in enumerate(lines):
            ...
            m_decl = re.search(r"\b(double|float)\s+(\w+)\s*[;=]", clean)
