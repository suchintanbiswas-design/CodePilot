...
class CMemoryAnalyzer:
    ...
    def analyze(self, code: str) -> List[Dict[str, Any]]:
        ...
        live_allocs: set[str] = (
            set()
        )  # root pointer names holding active heap allocations
        ...
        state_snapshots = (
            {}
        )  # depth -> (freed_roots, freed_ptrs, unchecked_ptrs, live_allocs, escaped_allocs)
        ...
        for i, line in enumerate(lines):
            ...
            if self._is_function_def(clean):
                ...
                for root in live_allocs:
                    if (
                        root not in freed_roots
                        and root not in escaped_allocs
                        and root not in leaked_allocs
                    ):
                        issues.append(
                            self._make_issue(
                                line_num - 1,
            ...
            if "}" in clean:
                ...
                if (brace_depth + 1) in diverging_depths:
                    if brace_depth in state_snapshots:
                        saved_roots, saved_ptrs, _, saved_live, saved_escaped = (
                            state_snapshots[brace_depth]
                        )
            ...
            for root in list(live_allocs):
                if root not in escaped_allocs and self._escapes_on_line(
                    root, ptr_alias, clean
                ):
            ...
            if m_alloc:
                ...
                if (
                    ptr in live_allocs
                    and ptr not in freed_roots
                    and ptr not in escaped_allocs
                    and ptr not in leaked_allocs
                ):
                    issues.append(
                        self._make_issue(
                            line_num,
            ...
            if m_free:
                ...
                if root in freed_roots:
                    ...
                    for alias_name, _alias_src in list(ptr_alias.items()):
    ...
    def _escapes_on_line(self, root: str, ptr_alias: dict[str, str], line: str) -> bool:
        ...
        aliases = [
            k for k, v in ptr_alias.items() if self._resolve_root(k, ptr_alias) == root
        ]
        aliases.append(root)
        ...
        for a in aliases:
            ...
            for m in re.finditer(
                rf"([a-zA-Z_]\w*)\s*\([^)]*\b{re.escape(a)}\b[^)]*\)", line
            ):
    ...
    def _is_used_on_line(self, ptr: str, line: str) -> bool:
        ...
        for m in re.finditer(
            rf"([a-zA-Z_]\w*)\s*\([^)]*\b{re.escape(ptr)}\b[^)]*\)", line
        ):
            if m.group(1) not in self._C_KEYWORDS:
                return True
        return False
